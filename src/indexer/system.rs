use crate::streams::transaction::ws_transaction_stream;
...
pub async fn transactions() -> Result<()> {
    let (provider_system_tx, provider_system_rx) = mpsc::unbounded_channel();
    let mut provider_system = ProviderSystem::new(provider_system_tx);
    ...
    provider_system.add_provider_stream(
        "polkachu-ws",
        ws_transaction_stream("wss://juno-testnet-rpc.polkachu.com/websocket"),
    );
    ...
    let indexer_handle = tokio::spawn(async move {
        ...
        while let Ok(transaction) = dispatcher_rx.recv().await {
            info!(
                "Indexing transaction index {} from height {}",
                transaction.index, transaction.height
            );
            indexer::index_transaction(&db, transaction).await?
        }
